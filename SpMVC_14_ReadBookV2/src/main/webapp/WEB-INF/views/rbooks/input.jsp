...
/*
	...
	conrolller의 GET method가 이를 수신하여 현재 input.jsp를 보여주고
	...
	button은 type을 지정하지 않으면 type="submit" 기 기본값이고
	버튼을 클릭하면 form에 입력한 데이터를 서버로 전송하는 기능을 한다.
	
	button에 특별히 이벤트를 적용하고 싶으면 
	type="button"으로 지정할수 있다
	
	button에는 3가지 type이 있다
	submit : 기본값이고 form안에 있는 경우 form에 담긴 데이터를 
			서버로 전송하는 기능
		input box가 1개만 있을 경우 Enter키에 반응하여
		버튼을 클릭한 것처럼 동작을 한다.
	button : 모든 기능을 제거하고 별도의 event를 설정해야 한다
	reset : form안에 있는 경우 form의 input box에 임의로 작성한
			데이터를 모두 초기화 하는 기능
	...
	<article>
		<form:form modelAttribute="rBookVO">
			...
			<div id="main-button">
				<button id="btn-write" 
				type="button"
				class="biz-orange flex-right">독서록 저장</button>

				<button id="btn-clear"
				type="reset" 
				class="biz-red">새로작성</button>

				<button id="btn-list" 
				type="button"
				class="biz-blue">리스트로 가기</button>
