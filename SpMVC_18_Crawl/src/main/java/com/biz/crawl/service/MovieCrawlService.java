...
import com.biz.crawl.persistance.NaverDao;
...
public class MovieCrawlService {
	...
	public List<NaverMovieVO> movieRankGet() {
		...
		// DOM ranListTag 문자열을 기준으로 리스트 추출
		...
		// rankList box들중에 상위 1부터 10번까지만 가져오기 수행
		for(int i = 0 ; i < 10 ; i++) {
			...
			String detailURL = "https://movie.naver.com" 
					+ titleList.get(i).attr("href");
			
			String code 
				= detailURL.substring(detailURL.lastIndexOf("=") + 1);
