...
public class TourController {
	...
	public String total(
			@RequestParam(value = "t_city", 
			required = false, 
			defaultValue = "1") String t_city,

			@RequestParam(value = "t_sigun", 
			required = false, 
			defaultValue = "1") String t_sigun,
			
			Model model) throws JsonSyntaxException, IOException {

		log.debug("메인 컨트롤러");
		
		List<CityVO> sigunList 
			= tService.getAreaData(t_city);
