...
.email-view div:nth-child(4) div,.email-view div:nth-child(5) div{
	width: 100%;
	padding: 10px 16px;
	background-color: #fff;
	color: black;
...
$(function(){
	...
	var buttonText = Array("수정","삭제","리스트보기")
...
<article class="email-view">
	...
	<div>
		<div>보낸Email</div>
		<div>${emailVO.from_email}</div>
		<div>보낸사람</div>
		<div>${emailVO.from_name}</div>
	</div>
	<div>
		<div>보낸날짜</div>
		<div>${emailVO.send_date}</div>
		<div>보낸시각</div>
		<div>${emailVO.send_time}</div>
	</div>
	<div>
